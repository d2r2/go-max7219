...
const MAX7219_LEDLINE_COUNT = MAX7219_REG_LASTDIGIT -
...
func NewDevice(cascaded int) *Device {
	buf := make([]byte, MAX7219_LEDLINE_COUNT*cascaded)
...
func (this *Device) sendBufferLine(position int) error {
	...
	for i := 0; i < this.cascaded; i++ {
		b := this.buffer[i*MAX7219_LEDLINE_COUNT+position]
...
func (this *Device) SetBufferLine(cascade int, position int, value byte, redraw bool) error {
	this.buffer[cascade*MAX7219_LEDLINE_COUNT+position] = value
	if redraw {
...
func (this *Device) Flush() error {
	for i := 0; i < MAX7219_LEDLINE_COUNT; i++ {
...
func (this *Device) Clear(cascade int) error {
	for i := 0; i < MAX7219_LEDLINE_COUNT; i++ {
		this.buffer[cascade*MAX7219_LEDLINE_COUNT+i] = 0
	}
	err := this.Flush()
	if err != nil {
		return err
